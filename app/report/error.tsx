...
export default function ReportError({ error, reset }: ErrorPageProps) {
  useEffect(() => {
    console.error("Report view error caught by boundary:", error);
  }, [error]);
      ...
      <div className="max-w-md w-full bg-card/80 dark:bg-[#0c0e14]/90 backdrop-blur-xl border border-border/60 dark:border-white/[0.07] rounded-3xl p-8 sm:p-10 shadow-2xl space-y-6 text-center">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-text-secondary">
            Unable to fetch or compile performance audit JSON artifacts for this report ID.
          </p>
        </div>

        {/* Monospace Error Detail Console */}
        <div className="text-left bg-surface-1/60 dark:bg-white/3 border border-border/60 dark:border-white/[0.07] rounded-2xl p-4 font-mono text-[11px] text-text-secondary overflow-x-auto space-y-1.5 max-h-36">
          <div className="text-score-poor font-bold">Error: {error.message || "Invalid report document references"}</div>
          {error.digest && <div className="opacity-75">Digest ID: {error.digest}</div>}
          <div className="opacity-60">Source: report/[id]/page.tsx</div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-2.5 pt-2">
          ...
          <Link
            href="/dashboard"
            className="flex-1 inline-flex items-center justify-center gap-1.5 h-10 px-4 rounded-xl text-xs font-semibold text-text-primary bg-surface-0/60 dark:bg-white/3 hover:bg-surface-1/60 dark:hover:bg-white/6 border border-border/60 dark:border-white/[0.07] transition-all cursor-pointer"
          >
