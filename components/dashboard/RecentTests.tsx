...
const RecentTests = ({ user }: { user: KindeUser }) => {
    ...
    return (
      <div className="space-y-4">
        {[1, 2].map((i) => (
          <div
            key={i}
            className="rounded-2xl bg-surface-0/70 dark:bg-white/3 backdrop-blur-md border border-border/60 dark:border-white/[0.07] p-6 shadow-2xs space-y-4"
          >
            <div className="flex items-center justify-between">
              <div className="space-y-2 flex-1 max-w-md">
                <div className="h-5 w-48 rounded-lg bg-surface-2 dark:bg-white/6 animate-pulse" />
                <div className="h-3.5 w-32 rounded-md bg-surface-2 dark:bg-white/6 animate-pulse" />
              </div>
              <div className="h-10 w-24 rounded-xl bg-surface-2 dark:bg-white/6 animate-pulse" />
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
              {[1, 2, 3, 4].map((j) => (
                <div key={j} className="h-16 rounded-xl bg-surface-1/80 dark:bg-white/4 animate-pulse" />
              ))}
            </div>
          </div>
        ))}
  ...
  if (error && storedList.length === 0) {
    return (
      <div className="text-center py-10 text-destructive font-mono text-xs bg-destructive/5 dark:bg-red-500/5 border border-destructive/20 dark:border-red-500/20 rounded-2xl p-6 backdrop-blur-md">
  ...
  if (displayTests.length === 0) {
    return (
      <div className="text-center py-12 px-6 bg-surface-0/70 dark:bg-white/3 backdrop-blur-md border border-border/60 dark:border-white/[0.07] rounded-2xl shadow-2xs space-y-3">
        <div className="h-12 w-12 rounded-2xl bg-surface-1/80 dark:bg-white/5 border border-border/40 dark:border-white/6 flex items-center justify-center mx-auto text-text-tertiary">
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3.5 bg-surface-0/70 dark:bg-white/3 backdrop-blur-md border border-border/60 dark:border-white/[0.07] p-3 sm:p-4 rounded-2xl shadow-2xs">
        {/* Left: View Mode Segmented Switcher */}
        <div className="flex items-center gap-2">
          <div className="inline-flex p-1 bg-surface-1/60 dark:bg-white/3 rounded-xl border border-border/40 dark:border-white/6">
            <button
              type="button"
            ...
            >
              ...
              <span className="ms-1 text-[10px] font-mono px-1.5 py-0.2 rounded-full bg-surface-2 dark:bg-white/6 text-text-tertiary">
                {domainGroups.length}
              </span>
            </button>
        ...
        <div className="flex items-center gap-2.5 flex-1 md:justify-end">
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="w-full bg-surface-1/70 dark:bg-white/4 border border-border/60 dark:border-white/8 rounded-xl ps-8 pe-3 py-1.5 text-xs font-mono text-text-primary placeholder:text-text-tertiary focus:outline-hidden focus:border-brand-500 focus:bg-surface-0 dark:focus:bg-white/[0.07] focus:ring-2 focus:ring-brand-500/20 transition-all"
            />
          </div>
          ...
          {completedTests.length >= 2 && (
            <Button
              ...
              className="h-8.5 px-3.5 text-xs font-semibold border-border/60 dark:border-white/8 bg-surface-0/60 dark:bg-white/4 hover:border-brand-300 hover:text-brand-600 dark:hover:text-brand-300 cursor-pointer gap-1.5 rounded-xl shrink-0 focus-ring"
            >
      ...
      {viewMode === "domain" && (
        <div className="space-y-4">
          {domainGroups.length === 0 ? (
            <div className="py-12 text-center text-xs text-text-tertiary font-mono bg-surface-0/70 dark:bg-white/3 backdrop-blur-md border border-border/60 dark:border-white/[0.07] rounded-2xl p-6">
