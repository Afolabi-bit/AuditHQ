...
export const DiagnosticsTab: React.FC<DiagnosticsTabProps> = ({
  ...
  if (!diagnostics || diagnostics.length === 0) {
    return (
      <div className="bg-surface-0/70 dark:bg-white/3 backdrop-blur-xl border border-border/60 dark:border-white/[0.07] rounded-3xl p-10 text-center text-text-secondary space-y-3 shadow-xs">
      ...
      <div className="space-y-3">
        {diagnostics.map((diag) => {
          ...
          return (
            <div
              key={diag.id}
              className={`bg-surface-0/70 dark:bg-white/3 backdrop-blur-xl border rounded-2xl overflow-hidden shadow-xs transition-all ${
                isExpanded
            ...
            >
              ...
              <div
                ...
                className="w-full p-4.5 sm:p-5 flex items-start justify-between gap-4 text-left cursor-pointer hover:bg-surface-1/40 dark:hover:bg-white/2 transition-colors select-none"
              >
                <div className="flex items-start gap-3.5 flex-1 min-w-0">
                  <div className="mt-0.5 p-2.5 rounded-xl bg-surface-1/80 dark:bg-white/4 text-text-secondary border border-border/50 dark:border-white/[0.07] shrink-0 shadow-2xs">
                ...
                <div className="flex items-center gap-2.5 shrink-0">
                  {onInspectItem && (
                    <button
                      ...
                      className="hidden sm:inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-xl bg-surface-1/80 dark:bg-white/4 hover:bg-surface-2 dark:hover:bg-white/8 border border-border/60 dark:border-white/[0.07] text-text-secondary hover:text-brand-600 dark:hover:text-brand-300 transition-all cursor-pointer shadow-2xs"
                  ...
                  <div className="p-2 rounded-xl bg-surface-1/80 dark:bg-white/4 border border-border/50 dark:border-white/[0.07] text-text-tertiary">
                    {isExpanded ? (
                        ...
                        <table className="w-full text-left border-collapse text-xs font-mono">
                          ...
                          <tbody className="divide-y divide-border/40 dark:divide-white/5">
                            {items.slice(0, 15).map((row, rIdx) => (
                              <tr key={rIdx} className="hover:bg-surface-1/50 dark:hover:bg-white/2 transition-colors">
                                {headings.map((h, hIdx) => {
